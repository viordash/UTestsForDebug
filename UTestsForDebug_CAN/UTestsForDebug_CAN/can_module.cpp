...
void CAN_SendCommand(TCommandId id, uint8_t status, uint8_t *payload) {
  ...
  if (HAL_CAN_AddTxMessage(&CanHandle, &header, (uint8_t *)&command,
                           &TxMailbox) != HAL_OK) {
...
bool CAN_HandleReceivingCommands(TCommandId *id, uint8_t *status,
  ...
  if (HAL_CAN_GetRxMessage(&CanHandle, CAN_RX_FIFO0, &header,
                           (uint8_t *)&command) != HAL_OK) {
    /* Reception Error */
    return false;
  }

  if ((header.StdId != 0x11) || (header.RTR != CAN_RTR_DATA) ||
      (header.IDE != CAN_ID_STD) || (header.DLC != sizeof(command))) {
    /* Rx message Error */
    return false;
  }
