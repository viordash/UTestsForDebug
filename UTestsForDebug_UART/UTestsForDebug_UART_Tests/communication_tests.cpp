...
TEST_GROUP(CommunicationTestGroup) {
	TEST_SETUP() {
		SystemClock_Config();
	}	
...
TEST(CommunicationTestGroup, Debug_VeryDifficult_Case) {	
	...
	CHECK_EQUAL_TEXT(LED6, led_On, "Perform_Command_VeryDifficult was not called");
	CHECK_EQUAL_TEXT(LED5, led_Off, "Perform_Command_VeryDifficult was not called");
...
void UART_SendCommand(TCommandId id, uint8_t status, uint8_t *payload) {
	...
	mock().actualCall("UART_SendCommand")
...
bool UART_HandleReceivingCommands(TCommandId *id, uint8_t *status, uint8_t *payload, size_t payload_size) {	
	...
	return mock().actualCall("UART_HandleReceivingCommands")
